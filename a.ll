...
entry:
  ...
  store float 1.000000e+01, float* %b, !dbg !15
  %a = getelementptr inbounds { float, float }, { float, float }* %s, i32 0, i32 0, !dbg !15
  store float %f32sum, float* %a, !dbg !15
  %s2 = load { float, float }, { float, float }* %s, !dbg !15
  %y = alloca { float, float }, !dbg !15
  store { float, float } %s2, { float, float }* %y, !dbg !15
  %s3 = getelementptr inbounds { float, float }, { float, float }* %y, i32 0, i32 0, !dbg !15
  store float 1.200000e+01, float* %s3, !dbg !15
  %y4 = load { float, float }, { float, float }* %y, !dbg !15
  ret { float, float } %y4, !dbg !15
}
